...
import { ReaderMobileBottomNav, ReaderMobileFab, ReaderMobileTools, ReaderMobileTopbar } from "@/components/reader/ReaderMobileControls";
...
export default function ReaderClient({ resourceId, courseId, studentId, title, allowDownload, appTimeZone }: { resourceId: string; courseId: string; studentId: string; title: string; allowDownload: boolean; appTimeZone: string }) {
  const pdfCanvasRef = useRef<HTMLCanvasElement>(null);
  const annotationCanvasRef = useRef<HTMLCanvasElement>(null);
  const pdfStageRef = useRef<HTMLDivElement>(null);
  const noteInputRefs = useRef<Record<string, HTMLTextAreaElement | null>>({});
  ...
  const suppressPdfTapUntilRef = useRef(0);
  ...
  const [pdfHasOverflow, setPdfHasOverflow] = useState(false);
  ...
  useEffect(() => {
    ...
    const touchViewport = window.matchMedia("(max-width: 1366px) and (pointer: coarse)");
    const update = () => setIsCompactReader(narrowViewport.matches || touchViewport.matches);
    update();
    narrowViewport.addEventListener("change", update);
    touchViewport.addEventListener("change", update);
    return () => {
      narrowViewport.removeEventListener("change", update);
      touchViewport.removeEventListener("change", update);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (!stage) return;
    const update = () => {
      setPdfStageWidth(stage.clientWidth);
      setPdfHasOverflow(stage.scrollWidth > stage.clientWidth + 2 || stage.scrollHeight > stage.clientHeight + 2);
    };
    update();
    const observer = new ResizeObserver(update);
    observer.observe(stage);
    return () => observer.disconnect();
  }, [isCompactReader, layoutMode]);
  ...
  }, [currentPage, isCompactReader, pdfLoading, pdfStageWidth, pdfZoom]);

  useEffect(() => {
    if (!tool || !pdfHasOverflow) setMobilePanMode(false);
  }, [pdfHasOverflow, tool]);

  useEffect(() => {
    if (!focusNoteId) return;
    const frame = window.requestAnimationFrame(() => {
      const input = noteInputRefs.current[focusNoteId];
      if (input) {
        input.focus();
    ...
    return () => window.cancelAnimationFrame(frame);
  }, [focusNoteId, mobileNotesOpen, notes]);
  ...
  useEffect(() => {
    ...
    if (isCompactReader && pdfStageWidth <= 0) return;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const fitWidth = Math.max(220, pdfStageWidth - 24);
        const displayScale = isCompactReader ? Math.max(0.1, fitWidth / baseViewport.width) * pdfZoom : 1.35;
        const pixelRatio = isCompactReader ? Math.min(window.devicePixelRatio || 1, 2) : 1;
        ...
        pdfCanvas.style.width = `${Math.ceil(displayViewport.width)}px`;
        pdfCanvas.style.height = `${Math.ceil(displayViewport.height)}px`;
        annotationCanvas.width = pdfCanvas.width;
        annotationCanvas.height = pdfCanvas.height;
        annotationCanvas.style.width = `${Math.ceil(displayViewport.width)}px`;
        annotationCanvas.style.height = `${Math.ceil(displayViewport.height)}px`;

        const pinch = pinchGestureRef.current;
        const stage = pdfStageRef.current;
        if (isCompactReader && pinch && stage && pinch.expiresAt > Date.now()) {
          const ratio = pdfZoom / pinch.startZoom;
          const nextLeft = (pinch.startScrollLeft + pinch.startCenterX) * ratio - pinch.currentCenterX;
          const nextTop = (pinch.startScrollTop + pinch.startCenterY) * ratio - pinch.currentCenterY;
          window.requestAnimationFrame(() => stage.scrollTo({ left: Math.max(0, nextLeft), top: Math.max(0, nextTop) }));
        }
        ...
        if (!cancelled) {
          setPdfLandscape(baseViewport.width > baseViewport.height);
  ...
  }, [currentPage, drawHighlights, isCompactReader, layoutMode, numPages, pdfStageWidth, pdfZoom]);
  ...
  function beginPinchGesture() {
    ...
    suppressPdfTapUntilRef.current = Date.now() + 450;
  }

  function handlePdfPointerDownCapture(event: React.PointerEvent<HTMLDivElement>) {
    if (!isCompactReader || event.pointerType !== "touch") return;
    touchPointersRef.current.set(event.pointerId, { x: event.clientX, y: event.clientY });
    if (touchPointersRef.current.size === 2) beginPinchGesture();
  }

  function handlePdfPointerMoveCapture(event: React.PointerEvent<HTMLDivElement>) {
    if (!isCompactReader || event.pointerType !== "touch" || !touchPointersRef.current.has(event.pointerId)) return;
    touchPointersRef.current.set(event.pointerId, { x: event.clientX, y: event.clientY });
    const pinch = pinchGestureRef.current;
    const pair = touchPair();
    if (!pinchActiveRef.current || !pinch || !pair) return;
    ...
    if (stage) {
      const rect = stage.getBoundingClientRect();
      pinch.currentCenterX = (first.x + second.x) / 2 - rect.left;
      pinch.currentCenterY = (first.y + second.y) / 2 - rect.top;
    }
    pinch.expiresAt = Date.now() + 700;
    const nextZoom = clampPdfZoom(pinch.startZoom * (distance / pinch.startDistance));
    if (Math.abs(nextZoom - pdfZoomRef.current) >= 0.01) {
      pdfZoomRef.current = nextZoom;
      setPdfZoom(nextZoom);
    }
    suppressPdfTapUntilRef.current = Date.now() + 450;
  }

  function handlePdfPointerEndCapture(event: React.PointerEvent<HTMLDivElement>) {
    if (!isCompactReader || event.pointerType !== "touch") return;
    touchPointersRef.current.delete(event.pointerId);
    if (pinchGestureRef.current && touchPointersRef.current.size < 2) {
      pinchActiveRef.current = false;
      pinchGestureRef.current.expiresAt = Date.now() + 700;
      suppressPdfTapUntilRef.current = Date.now() + 450;
    }
    if (touchPointersRef.current.size === 0) {
      window.setTimeout(() => {
        if (touchPointersRef.current.size === 0 && pinchGestureRef.current && pinchGestureRef.current.expiresAt <= Date.now()) {
          pinchGestureRef.current = null;
        }
      }, 750);
    }
  ...
  function pointerDown(event: React.PointerEvent<HTMLCanvasElement>) {
    if (!tool || mobilePanMode) return;
    if (event.pointerType === "touch" && (pinchActiveRef.current || touchPointersRef.current.size >= 2)) return;
    event.currentTarget.setPointerCapture(event.pointerId);
    activeStrokeRef.current = [canvasPoint(event)];
  }

  function pointerMove(event: React.PointerEvent<HTMLCanvasElement>) {
    if (!tool || !activeStrokeRef.current || pinchActiveRef.current) return;
    event.preventDefault();
  ...
  function toggleMobilePanMode() {
    if (!tool || !pdfHasOverflow) return;
    activeStrokeRef.current = null;
    setMobilePanMode((active) => !active);
    drawHighlights();
  }
  ...
  function changePdfZoom(delta: number) {
    setPdfZoom((value) => {
      const next = clampPdfZoom(value + delta);
      pdfZoomRef.current = next;
      return next;
    });
  }

  function openMobileJump() {
  ...
  function submitMobileJump(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const page = Number(mobileJumpPage);
  ...
  return <div className="reader-page" style={{ position: "fixed", inset: 0, zIndex: 20 }}>
    ...
    <div className="reader-body">
      {(isCompactReader || layoutMode !== "NOTES") && <section className="pdf-pane" style={{ width: isCompactReader || layoutMode === "PDF" ? "100%" : `${splitPercent}%` }}>
        <div className="reader-tools">
          <button className={`btn ${tool === "RED" ? "tool-active-red" : ""}`} onClick={() => setTool(tool === "RED" ? null : "RED")}>重點劃記（紅）</button>
          <button className={`btn ${tool === "YELLOW" ? "tool-active-yellow" : ""}`} onClick={() => setTool(tool === "YELLOW" ? null : "YELLOW")}>疑問劃記（黃）</button>
          ...
          <button className="btn" disabled={currentPageHighlightCount === 0} onClick={undoHighlight}>↶ 復原上一筆劃記</button>
          <button className="btn btn-danger" disabled={currentPageHighlightCount === 0} onClick={clearPageHighlights}>清除本頁劃記</button>
        ...
        {(mobileToolsOpen || Boolean(tool)) && <ReaderMobileTools
          tool={tool}
          brushSize={brushSize}
          brushSizeLabels={BRUSH_SIZE_LABELS}
          canUndo={currentPageHighlightCount > 0}
          canClear={currentPageHighlightCount > 0}
          zoom={pdfZoom}
          isLandscape={pdfLandscape}
          panMode={mobilePanMode}
          canPan={pdfHasOverflow}
          onSelectTool={selectMobileTool}
          onBrushSizeChange={setBrushSize}
          onUndo={undoHighlight}
          onClear={clearPageHighlights}
          onDoneDrawing={finishMobileDrawing}
          onTogglePan={toggleMobilePanMode}
          onZoomOut={() => changePdfZoom(-0.25)}
          onZoomReset={() => { pdfZoomRef.current = 1; setPdfZoom(1); }}
          onZoomIn={() => changePdfZoom(0.25)}
        />}

        <div
          className={`pdf-stage ${mobilePanMode ? "reader-pan-mode" : ""}`}
          ref={pdfStageRef}
          onClick={handlePdfStageTap}
        ...
        >
          ...
          <div className="canvas-wrap" style={{ display: pdfLoading ? "none" : "block" }}>
            ...
            <canvas ref={annotationCanvasRef} className="annotation-canvas" style={{ cursor: tool && !mobilePanMode ? "crosshair" : "default", pointerEvents: tool && !mobilePanMode ? "auto" : "none" }} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={finishStroke} onPointerCancel={finishStroke} />
          </div>
        </div>
        ...
        <ReaderMobileBottomNav
          ...
          understandingSaving={understandingSaving}
          pageNavigationLocked={pageNavigationLocked}
          pageGateMessage={pageGateMessage}
          trackingNotice={trackingNotice}
          errorMessage={error}
          onChooseUnderstanding={(status) => void chooseUnderstanding(status)}
          onPrevious={() => void navigateTo(currentPage - 1)}
          onNext={() => void navigateTo(currentPage + 1)}
          onOpenJump={openMobileJump}
        />
      </section>}

      {!isCompactReader && layoutMode === "SPLIT" && <div className="splitter" onMouseDown={startResize} title="拖曳調整PDF與筆記寬度" />}
      {!isCompactReader && layoutMode !== "PDF" && <aside className="notes-pane" style={{ width: layoutMode === "NOTES" ? "100%" : `${100 - splitPercent}%` }}>
    ...
    {mobileNotesOpen && <div className="reader-mobile-sheet-backdrop" onClick={() => setMobileNotesOpen(false)}>
      <section className="reader-mobile-sheet reader-mobile-notes-sheet" role="dialog" aria-modal="true" aria-label="我的筆記" onClick={(event) => event.stopPropagation()}>
        <div className="reader-mobile-sheet-handle" aria-hidden="true" />
        <div className="reader-mobile-sheet-head">
          <div><strong>我的筆記</strong><span className="badge">{notes.length}則</span></div>
          <button className="reader-mobile-icon-button" type="button" aria-label="關閉筆記" onClick={() => setMobileNotesOpen(false)}>×</button>
        </div>
        {restored && <div className="restore-banner">已恢復此瀏覽器上次未送出的暫存內容。</div>}
        {error && <div className="error reader-mobile-sheet-message">{error}</div>}
        <div className="notes-tabs reader-mobile-note-tabs"><button className={`btn ${noteTab === "CURRENT" ? "btn-primary" : ""}`} onClick={() => setNoteTab("CURRENT")}>本頁</button><button className={`btn ${noteTab === "ALL" ? "btn-primary" : ""}`} onClick={() => setNoteTab("ALL")}>全部筆記</button></div>
    ...
    {mobileJumpOpen && <div className="reader-mobile-sheet-backdrop" onClick={() => setMobileJumpOpen(false)}>
      <form className="reader-mobile-sheet reader-mobile-jump-sheet" onSubmit={submitMobileJump} onClick={(event) => event.stopPropagation()}>
        <div className="reader-mobile-sheet-handle" aria-hidden="true" />
        <div className="reader-mobile-sheet-head"><strong>跳至頁面</strong><button className="reader-mobile-icon-button" type="button" aria-label="關閉跳頁" onClick={() => setMobileJumpOpen(false)}>×</button></div>
        <label>頁碼
          <input type="number" min={1} max={numPages || 1} inputMode="numeric" value={mobileJumpPage} onChange={(event) => setMobileJumpPage(event.target.value)} autoFocus />
        </label>
        <div className="reader-mobile-jump-actions"><button className="btn btn-outline" type="button" onClick={() => setMobileJumpOpen(false)}>取消</button><button className="btn btn-primary" type="submit">前往</button></div>
      </form>
    </div>}

    {successAt && <div className="modal-backdrop"><div className="card modal stack"><h2 className="h2">作業已成功繳交</h2><div className="subtle">寫入時間：{new Intl.DateTimeFormat("zh-TW", { timeZone: appTimeZone, dateStyle: "medium", timeStyle: "medium" }).format(new Date(successAt))}</div><button className="btn btn-primary" onClick={() => setSuccessAt(null)}>繼續查看</button><Link className="btn btn-outline" href={`/courses/${courseId}`}>返回課程頁面</Link></div></div>}
  </div>;
}
