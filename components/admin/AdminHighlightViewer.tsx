...
export default function AdminHighlightViewer({ resourceId, title, students, highlights }: { resourceId: string; title: string; students: Student[]; highlights: Highlight[] }) {
  ...
  const allVisibleStudentsSelected = visibleStudents.length > 0 && visibleStudents.every((student) => selectedSet.has(student.id));
  ...
  function toggleVisibleStudents() {
    setSelectedIds((current) => {
      if (allVisibleStudentsSelected) return current.filter((id) => !visibleStudentIds.has(id));
      return Array.from(new Set([...current, ...visibleStudents.map((student) => student.id)]));
    });
  }
  ...
  return <section className="card admin-highlight-viewer">
    ...
    <div className="admin-highlight-body">
      ...
      <aside className="admin-highlight-students">
        ...
        <div className="row"><button className="btn btn-outline" type="button" disabled={visibleStudents.length === 0} onClick={toggleVisibleStudents}>{allVisibleStudentsSelected ? "取消全選" : "全選"}</button></div>
