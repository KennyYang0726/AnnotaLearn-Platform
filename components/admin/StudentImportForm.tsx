...
function statusLabel(status: PreviewStatus) {
  if (status === "NEW") return "新學生";
  if (status === "EXISTS") return "既有學生";
  ...
  return <div className="stack">
    ...
    {preview && <section className="card panel stack">
      ...
      {!imported && <div className="stack">
        {courses.length ? <div className="stack">
          <label>匯入後加入課程（選填）
          ...
          <div className="subtle">既有學生會沿用原帳號、不重設密碼，若選擇課程，也會正常加入該課程。</div>
        </div> : <div className="subtle">目前尚未建立課程，本次將只匯入學生。</div>}

        <div className="row">
          <button className="btn btn-primary" onClick={importStudents} disabled={!usableRows.length || busy !== null}>
            {busy === "IMPORT" ? "匯入中，正在處理學生名單…" : buttonLabel}
          </button>
        </div>
      </div>}
