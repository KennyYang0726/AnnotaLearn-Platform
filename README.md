...
- 學生 XLS/XLSX 名單採一次解析預覽；確認匯入時以多 Worker 平行建立 bcrypt 密碼雜湊，並可在同一流程批量加入指定課程
- 學生首次登入強制修改預設密碼
- PDF 資產庫，同一份 PDF 可被不同課程引用
- PDF.js 單頁閱讀器與頁碼切換
- 重點筆記與盲點提問
- 紅色螢光筆表示重點，黃色螢光筆表示疑問
- 劃記使用相對座標保存；文字擷取依實際筆跡命中的字元範圍判定，不再以整個 PDF TextItem 為單位
- 螢光筆提供 5 段粗細調整，新劃記會保存各自的筆跡粗細；舊劃記仍以原本粗細顯示
...
劃記以 0～1 的相對座標保存，因此不綁定特定螢幕尺寸。系統會將 PDF.js TextItem 進一步切分為字元級幾何範圍，再以實際螢光筆路徑判定命中的字元；因此同一個 TextItem 中只畫前兩字與只畫中間三字，會得到不同的擷取文字。
