...
export async function POST(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  const submittedAt = await prisma.$transaction(async (tx) => {
    ...
    if (data.highlights.length) await tx.highlight.createMany({ data: data.highlights.map((h) => ({ submissionId: submission.id, clientId: h.id, page: h.page, type: h.type, color: h.color, extractedText: h.extractedText || null, geometry: { points: h.points, ...(h.strokeWidthRatio ? { strokeWidthRatio: h.strokeWidthRatio } : {}) }, recordedAt: new Date(h.recordedAt) })) });
