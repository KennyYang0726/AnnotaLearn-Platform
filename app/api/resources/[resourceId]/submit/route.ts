...
const highlight = z.object({ id: z.string().min(1).max(100), page: z.number().int().positive(), type: z.enum(["IMPORTANT", "QUESTION"]), color: z.enum(["RED", "YELLOW"]), extractedText: z.string().max(5000).optional().nullable(), points: z.array(point).min(2).max(10000), recordedAt });
...
export async function POST(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  const submittedAt = await prisma.$transaction(async (tx) => {
    ...
    if (data.notes.length) await tx.note.createMany({ data: data.notes.map((n) => ({ submissionId: submission.id, clientId: n.id, page: n.page, type: n.type, content: n.content, recordedAt: new Date(n.recordedAt) })) });
    if (data.highlights.length) await tx.highlight.createMany({ data: data.highlights.map((h) => ({ submissionId: submission.id, clientId: h.id, page: h.page, type: h.type, color: h.color, extractedText: h.extractedText || null, geometry: { points: h.points }, recordedAt: new Date(h.recordedAt) })) });
