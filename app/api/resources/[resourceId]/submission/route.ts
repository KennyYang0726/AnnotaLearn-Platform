...
export async function GET(_request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  return NextResponse.json({ submission: { lastPage: submission.lastPage, submittedAt: submission.submittedAt, notes: submission.notes.map((n) => ({ id: n.clientId, page: n.page, type: n.type, content: n.content, recordedAt: n.recordedAt.toISOString() })), highlights: submission.highlights.map((h) => { const geometry = h.geometry as { points?: [number, number][]; strokeWidthRatio?: number }; return { id: h.clientId, page: h.page, type: h.type, color: h.color, extractedText: h.extractedText, points: geometry.points || [], strokeWidthRatio: typeof geometry.strokeWidthRatio === "number" ? geometry.strokeWidthRatio : undefined, recordedAt: h.recordedAt.toISOString() }; }) } });
}
