...
export async function GET(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  for (const enrollment of enrollments) {
    ...
    const common: CsvRow = [
      ...
      formatAppDateTime(resource.course.startAt),
      formatAppDateTime(resource.course.endAt),
      resource.title,
      ...
      submission ? "已繳交" : "尚未繳交",
      submission?.submittedAt ? formatAppDateTime(submission.submittedAt) : "",
      submission?.lastPage ?? "",
      visitedPages.length,
      completion ?? "",
    ...
    if (filters.recordType === "ALL" || filters.recordType === "NOTE") {
      for (const note of submission?.notes ?? []) {
        recordRows.push([...common, "文字筆記", note.type === "KEY_POINT" ? "重點" : "盲點提問", note.page, "", "", "", formatAppDate(note.recordedAt), note.content, "", ""]);
      }
    }
    ...
    if (filters.recordType === "ALL" || filters.recordType === "HIGHLIGHT") {
      for (const highlight of submission?.highlights ?? []) {
        ...
        recordRows.push([
          ...
          "",
          "",
          formatAppDate(highlight.recordedAt),
    ...
    if (filters.recordType === "ALL" || filters.recordType === "UNDERSTANDING") {
      for (const event of userEvents) {
        recordRows.push([...common, "理解狀態歷程", understandingLabel(event.status), event.page, "", "", "", formatAppDateTime(event.recordedAt), "", "", ""]);
      }
    }
