...
import { localStorageRoot, safeLocalFilename } from "@/lib/storage";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const auth = await requireApiTa(); if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });
  ...
  const asset = await prisma.asset.create({ data: { originalName: file.name, displayName: displayName || file.name.replace(/\.pdf$/i, ""), storageProvider: "local", storageKey, fileSize: file.size, pageCount, mimeType: "application/pdf", createdById: auth.user.id } });
