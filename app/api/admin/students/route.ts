...
const schema = z.object({
  studentId: z.string().trim().min(1).max(50).regex(/^[A-Za-z0-9_-]+$/, "學號只能使用英文字母、數字、底線與連字號"),
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message || "學生資料格式錯誤" }, { status: 400 });
  ...
  const user = await prisma.user.create({
    data: {
      ...
      departmentGrade: parsed.data.departmentGrade || null,
    },
