...
export async function POST(request: Request) {
  ...
  try {
    ...
    const preview = rows.map((row) => {
      ...
      else if (existingMap.get(key) === "ADMIN") { status = "CONFLICT"; message = "此帳號已由管理員帳號使用"; }
      else if (existingMap.get(key) === "TA") { status = "CONFLICT"; message = "此帳號已由助教帳號使用"; }
      else if (existingMap.get(key) === "STUDENT") { status = "EXISTS"; message = "使用既有學生帳號，匯入時更新名單資料但不重設密碼，若選擇課程仍會加入該課程"; }
      seen.add(key);
