...
export default async function SubmissionCoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return <div className="stack">
    <div className="page-head">
      <Link href={`/ta/courses/${courseId}`} className="subtle">←返回課程工作區</Link>
        ...
        {course.resources.map((resource, index) => {
          ...
          return <article className="highlight-resource-item" key={resource.id}>
            ...
            <div className="record-actions">
              <a className="btn btn-outline btn-nowrap" href={`/api/ta/submissions/resources/${resource.id}/export`}>匯出CSV</a>
              <Link className="btn btn-primary btn-nowrap" href={`/ta/courses/${course.id}/submissions/${resource.id}`}>查看學生紀錄</Link>
            </div>
          </article>;
        })}
      </div>}
