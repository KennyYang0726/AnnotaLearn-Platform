...
export default async function SubmissionResourcePage({
  ...
  return <div className="stack">
    <div className="page-head">
      <Link href={`/ta/courses/${courseId}/submissions`} className="subtle">←返回教材列表</Link>
    ...
    <section className="card panel">
      ...
      {students.length === 0 ? <div className="subtle">目前沒有符合篩選條件的學生紀錄。</div> : <div className="table-wrap"><table>
        ...
        <tbody>{students.map(({ enrollment, submission, visitedPages, browsingProgress, notUnderstoodPages, daily, resourceDaily, totalDuration, latest }) => <tr key={enrollment.userId}>
          ...
          <td><Link className="btn btn-primary btn-compact btn-nowrap" href={`/ta/courses/${courseId}/submissions/${resourceId}/student/${enrollment.userId}${querySuffix}`}>查看紀錄</Link></td>
        </tr>)}</tbody>
      </table></div>}
    </section>
  </div>;
