...
export default async function StudentReadingRecordPage({
  ...
  return <div className="stack">
    <div className="page-head">
      <Link href={`/ta/courses/${courseId}/submissions/${resourceId}${querySuffix}`} className="subtle">←返回學生紀錄</Link>
      ...
      <div className="subtle" style={{ marginTop: 4 }}>繳交狀態：{submission ? `已繳交（${formatAppDateTime(submission.submittedAt)}）` : "尚未繳交，閱讀行為仍會即時記錄"}</div>
    </div>
      ...
      <details className="understanding-history" style={{ marginTop: 14 }} open={filters.recordType === "DAILY_ACTIVITY"}>
        ...
        <div style={{ marginTop: 12 }}>
          {dailyActivities.length === 0 ? <div className="subtle">目前檢視期間內沒有學習活動日。</div> : <div className="activity-day-list">
            {dailyActivities.map((activity) => <span className="badge" key={activity.id}>{formatDbDateOnly(activity.activityDate)}</span>)}
          </div>}
        </div>
      </details>
    </section>
    ...
    <StudentReadingRecordViewer
      ...
      notes={(submission?.notes ?? []).map((note) => ({ id: note.id, type: note.type, page: note.page, content: note.content, recordedAt: formatAppDate(note.recordedAt) }))}
      highlights={(submission?.highlights ?? []).map((highlight) => {
        const geometry = highlight.geometry as Geometry;
        return { id: highlight.id, color: highlight.color, page: highlight.page, extractedText: highlight.extractedText || "", pointCount: Array.isArray(geometry.points) ? geometry.points.length : 0, recordedAt: formatAppDate(highlight.recordedAt) };
      })}
      visits={visitRows}
    />
  </div>;
