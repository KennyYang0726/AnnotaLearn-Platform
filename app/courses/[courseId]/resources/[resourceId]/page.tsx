...
export default async function ReaderPage({ params }: { params: Promise<{ courseId: string; resourceId: string }> }) {
  ...
  const resource = await prisma.courseResource.findFirst({ where: { id: resourceId, courseId } });
  if (!resource) notFound();
  return <ReaderClient resourceId={resource.id} courseId={courseId} studentId={student.id} title={resource.title} />;
}
