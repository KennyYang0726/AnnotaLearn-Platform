  ...
  return <div className="stack">
    <div className="page-head">
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div className="subtle">課程期間：{formatAppDateTimeDisplay(course.startAt, { dateStyle: "medium", timeStyle: "short" })}～{formatAppDateTimeDisplay(course.endAt, { dateStyle: "medium", timeStyle: "short" })}</div>
      </div>
    </div>
      ...
      {course.resources.length === 0 ? <div className="subtle">目前尚無教材。</div> : course.resources.map((r, index) => {
        ...
        return <div className="resource-row between" key={r.id}>
          <div>
            ...
            <div className="subtle">{r.asset.pageCount ? `${r.asset.pageCount}頁` : "PDF"} / {submission ? `已繳交 ${formatAppDateTimeDisplay(submission.submittedAt, { dateStyle: "short", timeStyle: "short" })}` : "尚未繳交"}</div>
          </div>
          <div className="row">
