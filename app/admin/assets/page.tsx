...
export default async function AssetsPage() {
  const assets = await prisma.asset.findMany({
    include: { createdBy: { select: { username: true, displayName: true, role: true } }, _count: { select: { resources: true } } },
      ...
      <div className="grid-2">
        ...
        <section className="card panel">
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>名稱</th><th>頁數</th><th>大小</th><th>上傳者</th><th>使用課程</th><th className="action-column">操作</th></tr></thead>
              <tbody>
                {assets.map((asset) => {
                  ...
                  return (
                    <tr key={asset.id}>
                      ...
                      <td>{asset.createdBy ? (asset.createdBy.role === "TA" ? `${asset.createdBy.displayName || asset.createdBy.username}（助教）` : asset.createdBy.displayName || asset.createdBy.username) : <span className="subtle">既有/系統資產</span>}</td>
                      <td>{usageCount}</td>
                      <td className="action-column">
                        <div className="row" style={{ flexWrap: "nowrap", gap: 8 }}>
                          ...
                          dangerMessage={usageCount > 0 ? `此資源目前正在被${usageCount}個課程使用。刪除後會同時從這些課程移除教材，且相關學生的繳交、文字筆記、PDF劃記、理解狀態與閱讀停留紀錄也會一併刪除。` : undefined}
                          />
                        </div>
                      </td>
                    </tr>
