...
export function recordedAtWhere(filters: ActivityFilters) {
  ...
  if (filters.from) where.gte = appDateRangeStart(filters.from) ?? undefined;
  if (filters.to) where.lte = appDateRangeEnd(filters.to) ?? undefined;
...
export { formatAppDate, formatAppDateTime };
